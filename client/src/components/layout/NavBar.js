  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-dark-800 border-t border-dark-700 pb-safe shadow-lg z-50">
      ...
      <div className="flex justify-around items-center py-3">
        ...
        <Link to="/profile" className={`flex flex-col items-center transition-all duration-200 hover:opacity-80 ${isActive('/profile')}`}>
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${location.pathname === '/profile' ? 'stroke-accent-500' : 'stroke-current'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">

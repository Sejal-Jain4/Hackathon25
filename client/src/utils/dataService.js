...
const STORAGE_KEYS = {
  ...
  DATA_LOADED: 'centsi_data_loaded',
  DATA_UPLOADED: 'centsi_data_uploaded'
};
...
export const addSavingsGoal = (goalData) => {
  ...
  // Check for multiple savings goals achievement - use the new length for accurate tracking
  const goalsCount = financialData.savingsGoals.length;
  const result = updateAchievementProgress('savings_goals_creator', { 
    newSavingsGoal: true,
    // Don't override the count, let the incremental counting work 
    // since we're adding one goal at a time here
  });
...
export const removeSavingsGoal = (index) => {
  ...
  // Important: We're not updating achievements here to avoid reverting completed achievements
  // The achievement for multiple savings goals will stay completed even if goals are deleted
...
/**
 * Clear data load state - use this when user logs out or when 
 * explicitly resetting the app state to simulate a new user flow
 */
...
/**
 * Check if data has been loaded by the user explicitly clicking "Upload Data"
 */
...
export const DEMO_ACHIEVEMENTS = [
  ...
  },
  {
    ...
    icon: '📚', // Book icon that matches the learn page theme
  ...
  },
  {
    ...
    icon: '🎤', // Microphone icon that matches the AI coach theme
 ...
 * When resetIfNew is true, will check for new login status and reset if needed
 */
export const getAchievements = () => {
  const storedAchievements = localStorage.getItem(STORAGE_KEYS.ACHIEVEMENTS);
  
  // If we have stored achievements, use them
  if (storedAchievements) {
    return JSON.parse(storedAchievements);
  }
  
  // Otherwise initialize with default achievements
  const defaultAchievements = [...DEMO_ACHIEVEMENTS];
  
  // Save these default achievements to localStorage
...
export const updateAchievementProgress = (achievementId, progressData = {}) => {
  ...
  switch (achievement.criteria) {
    case 'multipleSavingsGoals':
      // If the achievement is already completed, don't change its status
      if (achievement.status === 'completed') {
        // Keep it completed regardless of current goal count
        break;
      }
      
      if (progressData.newSavingsGoal) {
        // If setCurrentGoals is provided, use it directly; otherwise increment
        if (progressData.setCurrentGoals !== undefined) {
          achievement.currentGoals = progressData.setCurrentGoals;
        } else {
        ...
        console.log(`Goal progress: ${achievement.currentGoals}/${achievement.requiredGoals}`);
      }
      break;
    ...
    case 'savingsAmount':
      // If the savings amount achievement is already completed, don't change its status
      if (achievement.status === 'completed') {
        // Keep it completed regardless of current savings amount
        break;
      }
...
export const checkAchievementsProgress = () => {
  ...
  if (financialData.savingsGoals) {
    ...
    // First check if the achievement is already completed
    const achievements = getAchievements();
    const goalSetterAchievement = achievements.find(a => a.id === 'savings_goals_creator');
    
    // Only update if not already completed and we have goals
    if (savingsGoalsCount > 0 && (!goalSetterAchievement || goalSetterAchievement.status !== 'completed')) {
      const result = updateAchievementProgress('savings_goals_creator', { 
        newSavingsGoal: true,
        setCurrentGoals: savingsGoalsCount // Use setCurrentGoals to explicitly set the count
      });
  ...
  if (financialData.savingsGoals) {
    // First check if the achievement is already completed
    const achievements = getAchievements();
    const savingsMilestoneAchievement = achievements.find(a => a.id === 'savings_milestone');
    
    // Only update if not already completed
    if (!savingsMilestoneAchievement || savingsMilestoneAchievement.status !== 'completed') {
...
export const recordLearningCompleted = () => {
  // Get current count of completed learning activities
  const learningCount = parseInt(localStorage.getItem('centsi_learning_completed_count') || '0');
